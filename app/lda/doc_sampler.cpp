...
void DocSampler::sample_one_doc(LDADoc& doc, std::vector<float>& word_topic_count, std::vector<float>& word_topic_updates, std::map<int, int>& find_start_word_topic_idx) {
    // clear previous topic_summary_
    std::fill(topic_summary_.begin(), topic_summary_.end(), 0);
    //int topic_summary_start  = find_start_idx[V_ * ];
    //int topic_summary_start  = find_start_word_topic_idx[V_ * K_];
    int topic_summary_start  = find_start_word_topic_idx[V_];
    for (int k = 0; k < K_; k++) {
    ...
    for (LDADoc::Iterator it(&doc); !it.IsEnd(); it.Next()) {
        ...
        //LOG(INFO) << "modify local cache and updates";
        //LOG(INFO) << "t_s.size(): " << topic_summary_.size() << " word_topic_count: "<<word_topic_count.size()        << " word_topic_updates: " << word_topic_updates.size();
        //LOG(INFO) << "t_c_start: " << topic_summary_start << " V_: "<< V_ << " K_: " << K_ << " old topic: " << old_topic;
        assert(old_topic < K_);
        assert(word_start < V_ * K_);
        ...
        //LOG(INFO) << "modified local cache an updates";
        ...
        // Sample the topic for token 'it' using the aux variables.i
        ...
        //LOG(INFO) << "modifye cache and update";
