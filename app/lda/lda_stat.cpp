...
double LDAStats::ComputeWordLLH(int word_idx_start, int word_idx_end, std::vector<float>& word_topic_table, std::map<int, int>& find_start_idx) {
    double word_llh = 0.;
    static double zero_entry_llh = GetLogGammaBetaOffset(0);
    for (int w = word_idx_start; w < word_idx_end; ++w) {
        int idx = find_start_idx[w * K_];
        int num_zeros = 0;
        for (int i=0; i<K_; i++) {
