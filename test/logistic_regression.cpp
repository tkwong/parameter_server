...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < nodes.size(); i++)
    {
        ...
        threads[i] = new std::thread([&nodes, i, samples]()
        {
            ...
            task.SetLambda([table_id, samples](const Info& info)
            {
                ...
                DataStore part_samples(samples.begin() + 10000 * info.worker_id, 
                                       samples.begin() + 10000 * (info.worker_id + 1));

                std::cout << "Worker " << info.worker_id << " got " << part_samples.size()
                          << " samples." << std::endl;
                ...
                for (Sample sample : part_samples)
                {
                    // Pull
                ...
                // Test
                DataStore test_data(samples.begin()+30000, samples.end());
                std::vector<double> vals;
                table.Get(keys, &vals);
                ...
                std::cout << "Accuracy: " << correct << " out of " << test_data.size() 
                          << " " << float(correct)/test_data.size()*100 << " percent" 
                          << std::endl;
            });
